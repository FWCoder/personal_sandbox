...
def displayMACD(df, symbols, title="MACD Strategy", xlabel="Date", ylabel="Stock Prices"):
    ...
    while index < len(df.index):
        ...
        print df['MACD'][index - 1]
        print df['Signal'][index - 1]
        print df['MACD'][index]
        print df['Signal'][index]
        print "\n"
        if(df['MACD'][index - 1] < df['Signal'][index - 1] and df['MACD'][index] > df['Signal'][index]):
            plt.axvline(df.index[index], color='g')
            currentLong += 200
            file.write(str(df.index[index].date())+","+symbols+",BUY," + str(200) +"\n")
            currentShort = 0
            # isBuy = True
            # isSell = False
            #long_entry = True
...
def test_run():
    ...
    createMACD('IBM', start_date, end_date, 6, 10, 10000)   
